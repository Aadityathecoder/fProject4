...
MOTOR_SPEED = int(os.environ.get("ROBOT_MOTOR_SPEED", "13"))
TURN_INTENSITY = int(os.environ.get("ROBOT_TURN_INTENSITY", "12"))
...
API_HOST = os.environ.get("ROBOT_API_HOST", "172.20.140.15")
API_PORT = os.environ.get("ROBOT_API_PORT", "5000")
API_BASE_URL = os.environ.get("ROVER_API_BASE", f"http://{API_HOST}:{API_PORT}").rstrip("/")
API_STATUS_URL = f"{API_BASE_URL}/status"
API_UPLOAD_URL = f"{API_BASE_URL}/upload_frame"
DEBUG = os.environ.get("ROBOT_DEBUG", "1") != "0"

print(f"Using rover API: {API_BASE_URL}")
...
def execute_command(command):
    if DEBUG:
        print(f"Executing manual command: {command}")
    if command == 'backward':
        ...
        Motor.MotorRun(0, 'backward', max(0, MOTOR_SPEED - TURN_INTENSITY))
        Motor.MotorRun(1, 'backward', min(100, MOTOR_SPEED + TURN_INTENSITY))
    elif command == 'right':
        Motor.MotorRun(0, 'backward', min(100, MOTOR_SPEED + TURN_INTENSITY))
        Motor.MotorRun(1, 'backward', max(0, MOTOR_SPEED - TURN_INTENSITY + 2))
    else:
...
while True:
    ...
    try:
        ...
        now = time.time()
        if DEBUG and now - last_debug_print > 0.75:
            print(
                "Status from API: "
                f"manual={data.get('manual_command')} "
                f"autonomous={data.get('autonomous')} "
                f"auto_command={data.get('auto_command')} "
                f"auto_error={data.get('auto_error')} "
                f"auto_speed={data.get('auto_speed')} "
                f"robot_state={data.get('robot_state')}"
            )
        ...
        if data.get("martian_detected") or data.get("robot_state") in ("MARTIAN_DETECTED", "MARTIAN_STOPPED"):
            ...
            if auto_cmd == "stop" or base_speed < MIN_AUTO_SPEED:
                ...
                (
                    data.get("robot_state") == "PAPER_AVOIDANCE"
                    and avoidance_phase in ("avoid_right_until_clear", "avoid_left_recover")
                )
                or data.get("robot_state") == "LANE_RECOVERY"
            ) and auto_cmd in ("left", "right"):
                # Hard pivots during FOV-driven obstacle avoidance and lane-loss
                # recovery. The API follows these with straight forward motion.
                if auto_cmd == "right":
