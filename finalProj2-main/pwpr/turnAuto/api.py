...
LANE_RECOVERY_LEFT_FRAMES = 34
LANE_RECOVERY_FORWARD_FRAMES = 26
...
PAPER_RECOVER_LEFT_FRAMES = 28
PAPER_REENTER_FORWARD_FRAMES = 24
PAPER_TURN_ERROR = 180
PAPER_COOLDOWN_FRAMES = 25
PAPER_REARM_CLEAR_FRAMES = 10
PAPER_CLEAR_FOV_FRAMES = 2
...
def compute_auto_command(path, w):
    ...
    global lane_status, last_steering_cmd, auto_error, smoothed_error, lost_frame_count, auto_speed_scale, search_cycle, robot_state
    frame_center = w // 2
    ...
    cycle_len = LANE_RECOVERY_LEFT_FRAMES + LANE_RECOVERY_FORWARD_FRAMES
    phase = (search_cycle - 1) % cycle_len

    if phase < LANE_RECOVERY_LEFT_FRAMES:
        auto_speed_scale = ACQUIRE_PIVOT_SPEED
        auto_error = -SEARCH_ERROR
        lane_status = "Acquiring lane | course correcting left"
        return "left", None, auto_error

    auto_speed_scale = ACQUIRE_FORWARD_SPEED
    auto_error = 0
    lane_status = "Acquiring lane | forward after left correction"
...
def start_paper_avoidance(result, frame_width):
    ...
    # FOV-driven obstacle bypass: pivot right while the paper is still visible,
    # then turn left back toward the path as soon as the ball leaves the camera.
    avoidance_turn_error = PAPER_TURN_ERROR
    avoidance_phase = "avoid_right_until_clear"
    avoidance_counter = 0
...
def compute_paper_avoidance_command():
    ...
    if avoidance_phase == "avoid_right_until_clear":
        auto_command = "right"
        auto_error = PAPER_TURN_ERROR
        auto_speed_scale = 0.72
        lane_status = "Paper avoidance | turning right until FOV clear"
    elif avoidance_phase == "avoid_left_recover":
        auto_command = "left"
        auto_error = -PAPER_TURN_ERROR
        auto_speed_scale = 0.72
        lane_status = "Paper avoidance | ball clear, turning left to path"
        avoidance_counter -= 1
        if avoidance_counter <= 0:
            avoidance_phase = "avoid_forward_reenter"
            avoidance_counter = PAPER_REENTER_FORWARD_FRAMES
    elif avoidance_phase == "avoid_forward_reenter":
        auto_command = "forward"
        auto_error = 0
        auto_speed_scale = 0.82
        lane_status = "Paper avoidance | driving back onto path"
        avoidance_counter -= 1
        if avoidance_counter <= 0:
            avoidance_phase = "idle"
            avoidance_counter = 0
...
def process_frame(frame):
    ...
    if paper_ball_detected:
        ...
        if paper_rearm_required and avoidance_phase == "idle":
            # Same ball is still visible after the scripted maneuver. Do not
            # immediately restart the right/left bypass around the same object.
            pass
    elif paper_rearm_required:
        paper_clear_frames += 1
        if avoidance_phase == "avoid_right_until_clear" and paper_clear_frames >= PAPER_CLEAR_FOV_FRAMES:
            avoidance_phase = "avoid_left_recover"
            avoidance_counter = PAPER_RECOVER_LEFT_FRAMES
